...
function clearMap() { console.log('clear mini map')
    ...
    console.log(miniMap.ground.children.length)
    miniMap.attackers = []
...
class MiniMap extends Container {
    ...
    startAttackWave() {
        for (let unitType in state.player.attack) {
            while (state.player.attack[unitType].count) {
                state.player.attack[unitType].count--
...
EventHub.on( events.startAttackWave, () => miniMap.startAttackWave() )
